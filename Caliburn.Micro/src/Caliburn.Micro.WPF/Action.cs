using System.Windows;

namespace Caliburn.Micro
{
    public static class Action
    {
        static readonly ILog Log = LogManager.GetLog(typeof(Action));

        #region Target

        /// <summary>
        /// Gets the target for instances of <see cref="ActionMessage"/>.
        /// </summary>
        /// <param name="d">The element to which the target is attached.</param>
        /// <returns>The target for instances of <see cref="ActionMessage"/></returns>
        public static object GetTarget(DependencyObject d) => (object)d.GetValue(TargetProperty);

        /// <summary>
        /// Sets the target of the <see cref="ActionMessage"/>.
        /// </summary>
        /// <param name="d">The element to attach the target to.</param>
        /// <param name="value">The target for instance of <see cref="ActionMessage"/></param>
        public static void SetTarget(DependencyObject d, object value) => d.SetValue(TargetProperty, value);

        /// <summary>
        /// A property definition representing the target of an <see cref="ActionMessage"/>.
        /// The DataContext of the element will be set to this instance.
        /// </summary>
        public static DependencyProperty TargetProperty = DependencyProperty.RegisterAttached("Target", typeof(object), typeof(Action), new PropertyMetadata(OnTargetChanged));

        private static void OnTargetChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            SetTargetCore(e, d, true);
        }

        #endregion

        #region TargetWithoutContext

        /// <summary>
        /// Gets the target for instances of <see cref="ActionMessage"/>.
        /// </summary>
        /// <param name="d">The element to which the target is attached.</param>
        /// <returns>The target for instance of <see cref="ActionMessage"/></returns>
        public static object GetTargetWithoutContext(DependencyObject d) => d.GetValue(TargetWithoutContextProperty);

        /// <summary>
        /// Sets the target of the <see cref="ActionMessage"/>.
        /// </summary>
        /// <param name="d">The element to attach the target to.</param>
        /// <param name="value">The target for instance of <see cref="ActionMessage"/>.</param>
        /// <remarks>The DataContext will not be set.</remarks>
        public static void SetTargetWithoutContext(DependencyObject d, object value) => d.SetValue(TargetWithoutContextProperty, value);

        /// <summary>
        /// A property definition representing the target of an <see cref="ActionMessage"/>.
        /// The DataContext of the element is not set to this instanece.
        /// </summary>
        public static DependencyProperty TargetWithoutContextProperty = DependencyProperty.RegisterAttached("TargetWithoutContext", typeof(object), typeof(Action), new PropertyMetadata(OnTargetWithoutContextChanged));

        private static void OnTargetWithoutContextChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            SetTargetCore(e, d, false);
        }

        #endregion

        /// <summary>
        /// Checks if the <see cref="ActionMessage"/> Target was set.
        /// </summary>
        /// <param name="element">DependencyObject to check</param>
        /// <returns>True if Target or TargetWithoutContext was set on <paramref name="element"/></returns>
        public static bool HasTargetSet(DependencyObject element)
        {
            return (GetTarget(element) != null) || (GetTargetWithoutContext(element) != null);
        }

        private static void SetTargetCore(DependencyPropertyChangedEventArgs e, DependencyObject d, bool setContext)
        {
            if (Bootstraper.IsInDesignMode || e.NewValue == e.OldValue || e.NewValue == null)
                return;

            var target = e.NewValue;
            var containerKey = e.NewValue as string;

            if (containerKey != null)
                target = IoC.GetInstance(null, containerKey);

            if (setContext && d is FrameworkElement)
            {
                Log.Info($"Setting DC of {d} to {target}.");
                ((FrameworkElement)d).DataContext = target;
            }

            Log.Info($"Attaching message handler {target} to {d}.");
            Message.SetHandler(d, target);
        }
    }
}